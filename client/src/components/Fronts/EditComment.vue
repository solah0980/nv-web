<template>
  <div id="edit-comment">
    ...
    <transition name="fade">
      <p
        v-if="!editable"
        v-on:click.prevent="editComment(true, comment.id, compUser.id, user.id)"
      >{{comment.comment}}</p>
    </transition>
    <transition name="fade">
      <div v-if="updateEditable" class="form-edit-wrapper">
        <form v-on:submit.prevent="updateComment">
          <p>
            <textarea rows="5" class="form-control" v-model="comment.comment"></textarea>
          </p>
          <p>
            ...
            <button
              type="button"
              v-on:click.prevent="editComment(false, comment.id, compUser.id, user.id)"
    ...
    <p v-if="!editable && user != null && compUser.id === user.id">
